...
func TestEnsureFunctions(t *testing.T) {
	const pkg = "xpkg.crossplane.io/crossplane-contrib/function-patch-and-transform:v0.8.2"

	rev := &v1.CompositionRevision{
		ObjectMeta: metav1.ObjectMeta{Name: "example-rev", UID: "rev-uid"},
		Spec: v1.CompositionRevisionSpec{
			Pipeline: []v1.PipelineStep{
				{Step: "pnt", FunctionRef: v1.FunctionReference{Package: pkg}},
			},
		},
	}
	...
	t.Run("CreatesFunctionRevisionAndFunction", func(t *testing.T) {
		...
		if len(createdFn.Spec.ExternalRevisions) != 1 || createdFn.Spec.ExternalRevisions[0].Name != createdRev.GetName() {
			t.Errorf("created Function should reference the FunctionRevision as an external revision, got %+v", createdFn.Spec.ExternalRevisions)
		}
